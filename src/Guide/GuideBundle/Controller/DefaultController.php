...
use Guide\GuideBundle\GuideGuideBundle;
...
class DefaultController extends Controller
{
    ...
    public function showAction(Request $request)
    {
        ...
        /*---------------------on veut que dans les suggsettion d'amis nos amis ne vont pas figurer ---------*/
...
/*-------------------------------------------------------------------------------------*/
        $rando = new Randonne();
        $form2 = $this->createForm(RandonneType::class,$rando);
$form2->handleRequest($request);
        if($form2->isValid()&& $form2->isSubmitted())
        {
            $em=$this->getDoctrine()->getManager();
            $rando->setIdGuide($this->getUser()->getId());
            dump($rando);
            $em->persist($rando);
            $em->flush();

            foreach($all as $item9) {
                if($accutualuser->getId() != $item9->getId()){
                    $manager = $this->get('mgilet.notification');
                    $notif = $manager->generateNotification($this->getUser() . " vous invite à rejoindre son Evenement")
                        ->setLink('http://localhost/bakup14/web/app_dev.php/profile/showevent/'.$form2->get('type')->getData());
                    $manager->addNotification($item9, $notif);
        ...
        $randos = $em->getRepository('GuideGuideBundle:Randonne')->findAll();
...
/*--------------------------------------------------------------------------------------------------*/
        ...
        return $this->render('@GuideGuide/Default/guideprofile.html.twig', array(
            'user' => $user,'formm'=>$form8->createView(),'all'=>$users,'listinvi'=>$listinvitations,'listamis'=>$listamisuser
        ,'form5'=>$form5->createView()
        ,'form2'=>$form2->createView(),'chatForm' => $chatForm->createView(), 'users' => $listamisuser, 'getMessageUrl' => $getMessageUrl, 'currentUser' => $this->getUser()
,'randos'=>$randos,'form3'=>$form3->createView(),'groupes'=>$groups
    ...
    /**
     * @Route("/envoyerinvigui/{id}",name="envoyerinviguide")
     */
    ...
    /**
     * @Route("/app/{id}",name="approuverguide")
     */
